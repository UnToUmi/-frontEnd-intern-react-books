...
const BookModalCreate = (props) => {
    ...
    const onFinish = async (values) => {
        ...
        if (dataThumbnail.length === 0) {
            notification.error({
                ...
                description: "Please upload a book thumbnail"
            })
            return;
        }
        if (dataSlider.length === 0) {
            notification.error({
                message: "Validate Error",
                description: "Please upload book sliders"
            })
            return;
        }
    ...
    const handlePreview = async (file) => {
        if (file.url && !file.originFileObj) {
            ...
            setPreviewOpen(true);
            setPreviewTitle(title.name || file.url.substring(file.url.lastIndexOf('/') + 1))
            return;
        }
